...
$(function(){
	...
	$("#close").click(function(){
		let id = $("#uId").text();
		if($("#close").val() == "재입력"){
			$(opener.document).find("#uIdc").val("");
			window.close();
			opener.regFrm.uId.focus();    
		}else{
			$(opener.document).find("#uIdc").val(id);
			window.close();
			opener.regFrm.uId.focus();
		}
		// DOM으로 접근
	...
	$('#updateSbmBtn').click(function(){
		let chkQuit=confirm("수정하시겠습니까?");
		if(chkQuit){
			$('#regFrm').attr("action", "/memberModProc");
			$('#regFrm').submit();
		}
	});
